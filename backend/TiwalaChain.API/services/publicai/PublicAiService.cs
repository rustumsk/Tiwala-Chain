...
public sealed class PublicAiService
{
    ...
    public PublicAiService(IHttpClientFactory httpClientFactory, IMemoryCache memoryCache)
    {
        _httpClientFactory = httpClientFactory;
        _memoryCache = memoryCache;
    }

    public async Task<PublicAiResult> EvaluateFileAsync(
        IFormFile? file,
        string ip,
        CancellationToken cancellationToken)
    {
        if (file is null)
        {
            return PublicAiResult.BadRequest(new { error = "Upload a PDF or DOCX file to review." });
        }

        var validationError = ValidateUpload(file);
        if (validationError is not null)
        {
            return PublicAiResult.BadRequest(new { error = validationError });
        }

        if (!ActiveReviewByIp.TryAdd(ip, 0))
        {
            return PublicAiResult.TooManyRequests(new
            {
        ...
        try
        {
            ...
            var documentHash = Convert.ToHexString(SHA256.HashData(bytes)).ToLowerInvariant();
            if (_memoryCache.TryGetValue(GetCacheKey(documentHash), out PublicAiEvaluationResponse? cached) &&
                cached is not null)
            {
                return PublicAiResult.Ok(cached with { Cached = true });
            }
            ...
            if (!upstream.IsSuccessStatusCode)
            {
                ...
                return PublicAiResult.Status((int)upstream.StatusCode, new { error = message });
            }

            var response = BuildResponse(payload);
            _memoryCache.Set(GetCacheKey(documentHash), response, TimeSpan.FromHours(12));
            return PublicAiResult.Ok(response);
        }
        catch (TaskCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            return PublicAiResult.Status(StatusCodes.Status504GatewayTimeout, new
            {
        ...
        {
            return PublicAiResult.Status(StatusCodes.Status503ServiceUnavailable, new
            {
...
public sealed record PublicAiResult(int StatusCode, object? Payload, bool RetryAfter = false)
{
    public static PublicAiResult Ok(object? payload) => new(StatusCodes.Status200OK, payload);
    public static PublicAiResult BadRequest(object payload) => new(StatusCodes.Status400BadRequest, payload);
    public static PublicAiResult TooManyRequests(object payload) => new(StatusCodes.Status429TooManyRequests, payload, true);
    public static PublicAiResult Status(int statusCode, object payload) => new(statusCode, payload);
}
