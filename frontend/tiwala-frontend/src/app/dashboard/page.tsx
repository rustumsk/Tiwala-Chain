      ...
      <section className="mx-auto w-full max-w-[1580px] space-y-5">
        <section className={`${panelClass} px-6 py-6 lg:px-8 lg:py-7`}>
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className={`grid grid-cols-2 gap-x-6 gap-y-2 border-l pl-6 text-sm ${isDarkTheme ? "border-white/12" : "border-[#eceef5]"}`}>
              <span className={mutedTextClass}>{activeConfig.totalEscrowLabel}</span>
              <span className={`text-right font-semibold tabular-nums ${titleClass}`}>
                {formatUsdtValue(activeConfig.totalEscrow)} USDT
              </span>
              <span className={mutedTextClass}>Active workflow</span>
              <span className={`text-right font-semibold tabular-nums ${titleClass}`}>
                {activeConfig.activeJobs}
              </span>
              <span className={mutedTextClass}>Role mode</span>
              <span className={`text-right font-semibold capitalize ${titleClass}`}>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2 text-xs">
            <span className={`${chipClass} px-2.5 py-1.5`}>Wallet: {shortWallet}</span>
        ...
        (activeView === "freelancer" && showFreelancerList) ? (
          <section className={`${panelClass} p-6 lg:p-7`}>
            ...
            {activeConfig.isLoading ? (
              <p className={`text-sm ${mutedTextClass}`}>Loading jobs...</p>
